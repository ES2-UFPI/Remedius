...
const EditMedication = () => {
  ...
  const [medication, setMedication] = useState(
    () =>
      new MedicationPrototype(
        ...
        params.laboratorio as string,
        params.dataInicial ? new Date(params.dataInicial as string) : new Date(),
        params.horaInicial as string,
        params.frequencia as string,
        params.dosagem as string,
        params.quantidade as string,
        params.observacao as string,
        params.status as string | null,
        params.duracaoTratamento as string,
      ),
  )
  ...
  const onDateChange = ({ date }: { date: Date }) => {
    if (date && !isNaN(date.getTime()) && validateDate(date)) {
      setMedication((prev) => prev.clone().setDataInicial(date))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Update the medication for the user
      ...
      Alert.alert("Erro", "Não foi possível atualizar a medicação. Tente novamente.")
    } finally {
      setIsLoading(false)
    }
  }
            ...
            <View className="bg-white rounded-2xl p-4 sm:p-6 relative shadow-md">
                ...
                <View className="flex-row gap-2 mb-4">
                  <View className="flex-1">
                    ...
                    <TouchableOpacity className="bg-[#F3F3F3] p-3 rounded-lg" onPress={() => setShowDatePicker(true)}>
                      ...
                      {!medication.dataInicial && (
                        <Text className="text-red-500 text-sm mt-1">
                          Data inválida. Por favor, selecione uma nova data.
                        </Text>
                      )}
                    </TouchableOpacity>
                    <PaperProvider>
              ...
              <View className="flex-row gap-4">
                ...
                <TouchableOpacity
                  ...
                  disabled={isLoading}
                >
                  <Text className="text-white font-semibold text-lg">{isLoading ? "Atualizando..." : "Atualizar"}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
