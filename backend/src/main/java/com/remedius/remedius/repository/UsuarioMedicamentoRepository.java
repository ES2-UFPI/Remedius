...
public interface UsuarioMedicamentoRepository extends JpaRepository<UsuarioMedicamentoEntity, Integer> {
    ...
    List<UsuarioMedicamentoEntity> findByMedicamentoId(@Param("medicamentoId") Long medicamentoId);

    // Busca uma instância de relacionamento específica pelo ID do usuário e do medicamento
    @Query("SELECT um FROM UsuarioMedicamentoEntity um WHERE um.usuario.id = :usuarioId AND um.medicamento.id = :medicamentoId")
    UsuarioMedicamentoEntity findByUsuarioIdAndMedicamentoId(@Param("usuarioId") Long usuarioId, @Param("medicamentoId") Long medicamentoId);
}
