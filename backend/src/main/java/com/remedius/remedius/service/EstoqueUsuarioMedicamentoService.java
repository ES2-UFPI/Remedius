...
public class EstoqueUsuarioMedicamentoService {
    ...
    public EstoqueUsuarioMedicamentoEntity createEstoque(EstoqueUsuarioMedicamentoRequest estoqueRequest) {
        ...
        Optional<MedicamentoEntity> medicamentoOptional = medicamentoRepository
                .findById(estoqueRequest.getMedicamentoId());
        ...
        if (usuarioOptional.isPresent() && medicamentoOptional.isPresent()) {
            ...
            estoque.setDuracaoEstimada(calcularDuracaoEstoque(estoqueRequest));
    ...
    public Optional<EstoqueUsuarioMedicamentoEntity> updateEstoqueByUserMedicationId(
            EstoqueUsuarioMedicamentoRequest updatedEstoqueRequest) {
        EstoqueUsuarioMedicamentoEntity estoqueExistente = estoqueRepository
                .findByUserMedicationId(updatedEstoqueRequest.getUsuarioId(), updatedEstoqueRequest.getMedicamentoId());
        ...
        if (updatedEstoqueRequest.getQuantidade() != null) {
            estoqueExistente.setQuantidade(updatedEstoqueRequest.getQuantidade());
            estoqueExistente.setDuracaoEstimada(calcularDuracaoEstoque(updatedEstoqueRequest));
        }

        if (updatedEstoqueRequest.getUltimaCompra() != null) {
            estoqueExistente.setUltimaCompra(updatedEstoqueRequest.getUltimaCompra());
        }

        if (updatedEstoqueRequest.getStatus() != null) {
            estoqueExistente.setStatus(updatedEstoqueRequest.getStatus());
        }

        if (updatedEstoqueRequest.getDuracaoEstimada() != null) {
            estoqueExistente.setDuracaoEstimada(updatedEstoqueRequest.getDuracaoEstimada());
        }

        return Optional.of(estoqueRepository.save(estoqueExistente));
    }
